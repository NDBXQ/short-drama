...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  await db
    .insert(userSecurity)
    .values({ userId: id, tokenVersion: 2, updatedAt: now })
    .onConflictDoUpdate({
      target: userSecurity.userId,
      set: { tokenVersion: sql`${userSecurity.tokenVersion} + 1`, updatedAt: now }
    })
  ...
  await db.execute(sql`
    ...
    values (
      ...
      ${JSON.stringify({ id, account: user.account, email: user.email, isActive: user.isActive, tokenRevoked: true })}::jsonb,
      ${admin.ip},
      ${admin.userAgent},
      ${traceId}
    )
