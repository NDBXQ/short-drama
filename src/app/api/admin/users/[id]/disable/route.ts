...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  await db
    .insert(userSecurity)
    .values({ userId: id, tokenVersion: 2, disabledAt: now, disabledReason: reason, updatedAt: now })
    .onConflictDoUpdate({
  ...
  await db.execute(sql`
    ...
    values (
      ...
      ${JSON.stringify({ id, account: user.account, email: user.email, isActive: false, disabledAt: now.toISOString(), disabledReason: reason })}::jsonb,
      ${admin.ip},
      ${admin.userAgent},
      ${traceId}
    )
