...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  await db.update(users).set({ password: passwordHash, updatedAt: new Date() }).where(eq(users.id, id))

  const now = new Date()
  await db
    .insert(userSecurity)
    .values({ userId: id, tokenVersion: 2, passwordUpdatedAt: now, failedLoginCount: 0, lockedUntil: null, updatedAt: now })
    .onConflictDoUpdate({
  ...
  await db.execute(sql`
    ...
    values (
      ...
      ${JSON.stringify({ id, account: user.account, email: user.email, isActive: user.isActive, passwordUpdatedAt: now.toISOString(), tokenRevoked: true })}::jsonb,
      ${admin.ip},
      ${admin.userAgent},
      ${traceId}
    )
